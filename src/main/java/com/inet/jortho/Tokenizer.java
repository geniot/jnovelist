        ...
        while (true) {
            ...
            while (endWord != BreakIterator.DONE) {
                ...
                if (word.length() > 1 && Character.isLetter(word.charAt(0))) {
                    ...
                    if (!exist && bundle.existInDictionary(word.toLowerCase(), dictionary, options, isFirstWordInSentence)){
                        exist = true;
                    }

                    //handling cases of actual words that are connected with dash (so-so, yes-no, what-ever-you-mean-darling)
                    if (!exist && word.split("-").length > 1) {
                        boolean existLoc = true;
                        for (String splitWord : word.split("-")) {
                            if (!bundle.existInDictionary(splitWord, dictionary, options, isFirstWordInSentence)) {
                                existLoc = false;
                                break;
                            }
                        }
